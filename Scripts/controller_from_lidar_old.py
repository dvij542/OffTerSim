...
class controller:
  ...
  def callback(self,_):
    ...
    steering = pure_pursuit(ttl,(self.x,self.y,self.yaw))
    print(steering, "Done")
    # print(grid_pts)
    
    cmd.axes = [forward_speed,forward_speed,(steering+0.1)/max_steering,(steering+0.1)/max_steering,0.,0.]
    pub.publish(cmd)
    return
    for pt in pcl_raw :
      if pt[0] > 0. and pt[2] < 0.3 and pt[2] > 0.:
        angle = math.atan2(pt[1],pt[0])
        if abs(angle) > math.pi/2. :
          continue
        angle_t.append([math.sqrt(pt[0]**2+pt[1]**2),math.sqrt(pt[0]**2+pt[1]**2),angle,])
        if pt[1] > 0.1 :
          pcl_left.append([angle,pt[0],pt[2]])
        elif pt[1] < -0.1 :
          pcl_right.append([angle,pt[0],pt[2]])
        else :
          pcl_center.append([angle,pt[0],pt[2]])
    pcl_left = np.array(pcl_left)
    pcl_right = np.array(pcl_right)
    pcl_center = np.array(pcl_center)
    angle_t_sorted = np.array(angle_t)[:,::-1]
    # angle_t_sorted = sorted(angle_t,key=lambda x: x[:,0])
    angle_t_sorted = angle_t_sorted[angle_t_sorted[:,0].argsort()]
    # print(angle_t_sorted)
    # angle_t_sorted
    curr_angle = angle_t_sorted[0,0]
    curr_val = angle_t_sorted[0,1]
    for i,t in enumerate(angle_t_sorted) :
      val = t[1]
      angle = t[0]
      if val < thres :
        curr_angle = angle + inflation_dist/val
        curr_val = val
      if angle < curr_angle :
        angle_t_sorted[i,1] = min(curr_val,angle_t_sorted[i,1])

    curr_angle = angle_t_sorted[-1,0]
    curr_val = angle_t_sorted[-1,1]
    for i,t in enumerate(angle_t_sorted[::-1,:]) :
      val = t[1]
      angle = t[0]
      if val < thres :
        curr_angle = angle - inflation_dist/val
        curr_val = val
      if angle > curr_angle :
        angle_t_sorted[-i-1,1] = min(curr_val,angle_t_sorted[-i-1,1])
      angle_t_sorted[-i-1,1] += steer_cost*abs(angle) 
    # print(angle_t_sorted)

    minpos = np.argmax(angle_t_sorted[:,1])
    target_dir = angle_t_sorted[minpos,0]
    # print(angle_t_sorted)
    # print(np.min(pcl[:,0]),np.max(pcl[:,0]))
    # print(np.min(00000000pcl[:,1]),np.max(pcl[:,1]))
    # print(np.min(pcl[:,2]),np.max(pcl[:,2]))
    vals = np.array([np.min(pcl_left[:,1]),np.min(pcl_center[:,1]),np.min(pcl_right[:,1])])
    # print(vals)
    steer_code = np.argmax(vals)
    # return
    # cmd = Joy()
    cmd.header.stamp = now
    ...
    # steer_code = expert_policy(grid_pts)
    # steering = -(steer_code - 1)*max_steering
    steering = target_dir * steer_factor * max_steering/(math.pi/3.)
    print(steering, "Done")
    # print(grid_pts)
