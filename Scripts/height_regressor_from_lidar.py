...
# Load the ONNX model
POLICY_LOC = "/home/nsh1609/uneven-terrain-driver/results/without_lidar_attempt1/ArcadeDriver/ArcadeDriver-979983.onnx"
...
input_mask = np.random.rand(1, 5).astype(np.float32)*1.
...
env = UnityEnvironment(file_name="with_lidar/with_lidar", seed=2, side_channels=[])
env.reset()
...
while(True) :
    ...
    start_time = time.time()
    vector_observations = env_info[0].obs[0]
    heights = vector_observations[0,-55:]
